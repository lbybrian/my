...
export default {
  name: "",
  beforeMount() {
    this.produceRandomElements(200, 90);
  ...
  methods: {
    ...
    initChart() {
      ...
      this.cy.nodes().on("click", function(e) {
        ...
        _self.$emit("node-click", currentNode.data(), currentNode, cy, _self);
      }); 

       this.cy.edges().on("click", function(e) {
        var currentNode = e.target;
        _self.$emit("edge-click", currentNode.data(), currentNode, cy, _self);
      }); 
