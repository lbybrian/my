    ...
    >
      <slot v-for="(item, index) in dataInfo.data">
        ...
        >
		  ...
		  <el-button v-if="item.type==='button1'" @click="submitForm('dataInfo')">
		  	{{item.name}}
		  </el-button>
		  <el-button v-if="item.type==='button2'" @click="resetForm('dataInfo')">
		  	{{item.name}}
		  </el-button>
      </slot>
  	...
  	deepClone(source){
    		...
    		for(let keys in source){
    			if(source.hasOwnProperty(keys)){
    				...
    				}else if(source[keys]&&(typeof source[keys]=='string')){
    					obj[keys]=" "
    				}
    			}
    		}
...
//				this.deepClone(this.dataObj[key])
//			}
//			if(typeof this.dataObj[key]=='string'){
//				this.deepClone(this.dataObj[key])
//			}
//  	}
